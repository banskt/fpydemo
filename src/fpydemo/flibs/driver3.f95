...
!
      ...
      real(dp), allocatable  :: l(:), x(:), u(:), g(:), wa(:)
!
...
!     We now write the heading of the output.
      ...
      write (6,16) afact
  16  format(/,5x, 'Solving sample problem with a = ', f5.3, &
...
!               g(1) = 2.d0*(x(1)-1.d0)-1.6d1*x(1)*t1
               g(1) = 2.d0*(x(1)-1.d0) - 4.d0*afact*x(1)*t1
               do 22 i=2,n-1
                  t2=t1
                  t1=x(i+1)-x(i)**2
!                  g(i)=8.d0*t2-1.6d1*x(i)*t1
                  g(i) = 2.d0*afact*t2 - 4.d0*afact*x(i)*t1
  22           continue
!               g(n)=8.d0*t1
               g(n) = 2.d0*afact*t1
            endif
